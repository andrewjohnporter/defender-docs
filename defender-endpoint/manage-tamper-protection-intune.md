...
| Device management | Devices are either managed in Intune only, or are managed with Configuration Manager only. Sense must be enabled. Please can information be added here to list whether co-managed devices are defintely not supported for this. The text generlaly makes is sound like this is the case but I'm not 100% on this.
